...
function _update60()
  ...
  if is_title() then
    ...
    elseif btn(🅾️) or btn(❎) then
      music(-1)
      start_game_flash,start_game=100,true
      sfx(38)
    end
  end
end
...
function _draw()
  ...
  pal()
	...
	if is_title() then
		if start_game then
			for i=1,15 do
				pal(i, start_game_flash<=20 and ceil(max(start_game_flash)/10) or frames%20<10 and 7 or i)
			end
		end
  ...
  cls()
  rectfill(0,0,127,127,flash_bg and frames/10 or new_bg and 2 or 0)
  ...
  if lvl_id==#levels and objects[2].type==player then
...
function draw_time(x,y)
	...
	?two_digit_str(minutes\60)..":"..two_digit_str(minutes%60)..":"..two_digit_str(seconds\30)..":"..two_digit_str(round(seconds%60*100/30)),x+1,y+1,7
end
...
player={
  ...
  update=function(this)
    ...
    this.dash_effect_time=max(0,this.dash_effect_time-1)
    ...
    if this.dash_time>0 then
      ...
      -- x movement
      local maxrun=g_maxrun
      local accel=on_ground and g_gnd_accel or g_air_accel
      local deccel=g_deccel
      ...
      if not on_ground then
        this.spd.y=appr(this.spd.y,maxfall,abs(this.spd.y)>g_peak_thresh and g_grav or g_grav/2)
      elseif this.spd.y>0 then
      		this.spd.y=0
      end
      
      --fix float
      if this.is_solid(0,-1) and this.spd.y<0 then
        this.spd.y=appr(this.spd.y,maxfall,abs(this.spd.y)>g_peak_thresh and g_grav*1.3333 or g_grav/1.6666)
        if this.spd.x==0 then
        	this.dash_time=0
      ...
      if this.jbuffer>0 then
        if this.grace>0 then
          ...
          if wall_dir~=0 then
            psfx(2)
            this.jbuffer=0
            this.spd=vector(-wall_dir*(maxrun+g_wj_xboost),g_jump_spd)
      ...
      -- dash
      local d_full=g_dash_spd
      local d_half=g_dash_spd/sqrt(2)

      if this.djump>0 and dash then
        this.init_smoke()
        this.djump-=1
        this.dash_time=g_dash_time
        has_dashed=true
        ...
        this.dash_target_x=g_dash_target*sign(this.spd.x)
        this.dash_target_y=(this.spd.y>=0 and g_dash_target or g_dash_target_rising)*sign(this.spd.y)
        this.dash_accel_x=this.spd.y==0 and g_dash_accel or g_dash_accel/sqrt(2)
        this.dash_accel_y=this.spd.x==0 and g_dash_accel or g_dash_accel/sqrt(2)
    ...
    if this.y<-4 and lvl_id<#levels then
      next_level()
    end
  ...
  draw=function(this)
    -- clamp in screen
    local clamped=mid(0,this.x,lvl_pw)
...
function set_hair_color(djump)
  pal(8,djump==1 and 8 or djump==2 and 7+(frames\6)%2*4 or 12)
end
  ...
  update=function(this)
    ...
    elseif this.state==2 then
      ...
      if this.delay<0 then
        ...
        --local oldp=init_object(old_player,this.x,this.y)
        --oldp.move(16,0)
      end
    end
  end,
	...
	update=function(this)
		...
		if this.show and hit and this.delta<=1 then
			if this.dir==0 then
				...
				hit.spd=vector(this.dir*2.0666,-1.125)
			end
			hit.dash_time=0
...
smoke={
  init=function(this)
    this.spd=vector(0.15+rnd(0.1),-0.05)
    this.x+=-1+rnd(2)
    this.y+=-1+rnd(2)
  ...
  update=function(this)
    ...
    if has_dashed then
     if this.sfx_delay>0 then
      ...
      this.spd.y=appr(this.spd.y,-2.05,0.075)
      if this.y<-16 then
        destroy_object(this)
      end
    -- wait
...
fake_wall={
  ...
  draw=function(this)
    sspr(0,32,8,16,this.x,this.y,8,16)
...
big_chest={
  ...
  draw=function(this)
    if this.state==0 then
      ...
      sspr(0,48,8,8,this.x,this.y)
      sspr(0,48,8,8,this.x+8,this.y,8,8,true)
    elseif this.state==1 then
      this.timer-=1
      flash_bg=true
      if this.timer<=90 and #this.particles<50 and frames%2==0 then
        add(this.particles,{
          x=1+rnd(14),
          y=0,
          h=32+rnd(32),
    ...
    sspr(0,56,8,8,this.x,this.y+8)
    sspr(0,56,8,8,this.x+8,this.y+8,8,8,true,false)
  end
}
...
orb={
  ...
  draw=function(this)
    this.spd.y=appr(this.spd.y,0,0.125)
    local hit=this.player_here()
    ...
    for i=0,0.875,0.125 do
      circfill(this.x+4+cos(frames/60+i)*8,this.y+4+sin(frames/60+i)*8,1,7)
    end
  end
}
...
mapdata={
	[2]=""
}
...
g_maxrun=1 --run speed
g_gnd_accel=0.30775 --grounded accel
g_air_accel=0.16121 --midair accel
g_deccel=0.03943 --deaccel
g_maxfall=1.5 --max fall speed
g_wallslide=0.38922 --sliding fall speed
g_grav=0.09311 --gravity
g_peak_thresh=0.20121 --peak threshold
g_jump_spd=-1.75542 --jump speed
g_wj_xboost=0.53707 --walljump x boost
g_dash_spd=0.96093 --dash speed
g_dash_target=1.65640 --dash target
g_dash_target_rising=1.60363 --rising target
g_dash_accel=0.20328 --dash accel
g_dash_time=8 --dash time in frames 
__gfx__
...
__gff__

__map__

__sfx__
